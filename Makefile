...
up:
	...
	@echo "  apigee   → http://localhost:8080  (APIGEE sim + eBPF mirror agent, same container)"
	@echo "  ewp-sim  → http://localhost:8081  (Enterprise Web Proxy simulator)"
...
# ── Smoke tests ───────────────────────────────────────────────────────────────
test-traffic:
	@echo "=== Sending test requests to APIGEE (port 8080) ==="
	...
	curl -s http://localhost:8080/api/v1/users/user-42 | python3 -m json.tool
	@echo ""
...
report:
	...
	@curl -s http://localhost:8081/validation-report | python3 -m json.tool
	@echo ""
	@echo "=== EWP Metrics ==="
